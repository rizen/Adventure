package Adventure::Module::ActionCastle::Action::TrollIsFed;

sub main {
    my ($class, $params) = @_;
    if (Adventure->player->has_item('fish')) {
        Adventure->player->remove_item('fish');
        Adventure->player->location_object->property('troll_fed', 1);
        Adventure->player->location_object->remove_actor('troll');
        Adventure->player->announce('Satisfied, the troll leaves with his fish.');
    }
    else {
        Adventure->player->announce("You don't have a fish.");
    }
    return;
}

1;
